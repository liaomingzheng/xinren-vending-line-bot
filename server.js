...
function locationMessage() {
  const locationText =
    ...
    vendingMachines.map((machine, index) => {
      return (
        `${index + 1}. ${machine.name}（${machine.note}）\n` +
        `機台編號：${machine.id}\n` +
        `地址：${machine.address}\n` +
        `地圖：${machine.mapUrl}`
      );
  ...
  return [
    text(locationText),
    buttons("機台據點查詢", "請選擇要開啟的地圖", [
      uriAction("南投埔里據點", "https://maps.app.goo.gl/2bX5Xt6S8NXAEk9D7"),
      uriAction("高雄大樹據點", "https://maps.app.goo.gl/9x3eaFvodGGbGYf3A"),
